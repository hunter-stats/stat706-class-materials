...
Shown below are two diagnostic plots, the qnorm plot of the residuals, and the "calibration curve" i.e. the plot of the `profit` variable against the predicted profits for the dataset:
...
p0 <- ggplot(data = movies, aes(sample=residual)) + stat_qq() + stat_qq_line() 
...
The model formulation in this paper is ordinary least squares regression, passed through R's `step` function, which minimizes the AIC by removing one variable at a time. The `step` function was chosen as the primary technique for this model so as to "let the data speak"; that is, to determine the best predictors of `profit` without imposing too much speculation on model structure from the outset. 

For this reason, interaction effects between genres e.g. "action-comedy" or "musical-drama" were not accounted for. Doing so would entail arbitrary judgements about which interactions are valid "compound" genres; on the other hand, adding all pairwise interactions among 20 genres would add 190 additional predictors, with which comes numerous co-linearity problems and computational overhead.

## Interpretation

### Model Coefficients

Given a threshold of .05, all variables are significant in the final model with the exceptions of `release_date` and three genres -- `fantasy`, `family`, and `foreign`. 

The AIC minimization procedure eliminated 7 variables from the complete model, all of which are genres, the general trend being that niche genres such as `tv_movie` are removed in favor of mainstream ones such as `action`. The the predictors dropped by the `step` function typically had p-values well above .05 in the original model.
...
`horror` would be a counterexample to this conclusion, but it is difficult to ascertain why this is the case; further investigation might dive deeper into what characteristics of a film cause it to be labeled with a certain genre, perhaps via text analysis of the script. 

Not surprisingly, the remaining significant variables `budget` and `average_rating` all have positive effects on `profit`. 

The magnitudes of the coefficients are interesting as well. Genres all have coefficients on the order of magnitude `10e6`, with the exception of `western`, which has an effect on the order of `10e7`; a change in genre can make a difference on the order of $1,000,000$ in profit.

`budget` has an effect on the order of `10e0`, indicating a roughly 2:1 increase in profit for an added dollar of budget.`average_rating` has a particularly strong effect on `profit`, with a coefficient of `4.0986e+06`; a single star increase in rating entails an increase in profit of $4,000,000. 

The original question about `release_date` is difficult to answer, as its coefficient did not pass significance.

Altogether, the model suggests that the best way to make a profitable film is to create a lighthearted adventure film which is also a critical darling, such as Lord of the Rings or Star Wars. 
...
The model errors are "heavy-tailed" rather than normally distributed (see the results section). Though this has no pertinence for the sign and magnitude of our estimated coefficients, it does make our significance estimates and confidence intervals suspect. I attempted to solve this problem by restricting the analysis to films which were profit positive and log-transforming the `profit` variable, but this did not remediate this issue.
...
In fact the plots of above variables against the residuals closely resemble the plots of the variables against the response. This indicates the model is predicting zero for most films, likely being heavily driven by the vast majority of films which yield zero profit. As we can see below, predictions -- especially for highly profitable films -- regularly fall below the identity line.
```{r}
