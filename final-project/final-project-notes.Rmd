...
```{r}
ggplot(movies, aes(x=profit, y=budget)) + 
...
First, the model errors are "heavy-tailed" rather than normally distributed. 

```{r}
plot(lmod_reduced, which = 2)
```

Though this has no impact on the sign and magnitude of our estimated coefficients, it does make our significance estimates and confidence intervals suspect. 
...
The two problems above are due to the same underlying issue with the data -- the response is heavily peaked at zero, and so the regression has trouble picking up signal because it is largely encouraged to skew predictions towards zero. As we can see below, predictions, especially for highly profitable films, regularly fall closer to zero than should be the case. As well, it seems that the absolute value of the residual increases with profit, indicating our model has trouble dealing with larger response values.

```{r}
p1 <- ggplot(data = movies, aes(x=profit, y=prediction)) + geom_point() + geom_smooth() +  geom_abline(intercept = 0, slope = 1, color="red", 
                 linetype="dashed", size=1.5) 

p2 <- ggplot(data = movies, aes(x=profit, y=sqrt(residual^2))) + geom_point() + geom_smooth()
...
The primary improvement that should be done here is to change the link function to account for heavy tails. Perhaps robust regression would be a suitable technique here, or a link function that models an exponential response rather than a gaussian.

The effects of genres are somewhat difficult to interpret here. Genres themselves might be expressions of deeper underlying characteristics of films, and numerous external variables are not controlled for, e.g. marketing, the reputation of particular directors, and interactions among genres. Further investigation might also try to extract features from the "ground truth" e.g. film scripts or images.
